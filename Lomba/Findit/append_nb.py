...
new_cell_content = """### super train restart ###
# === SEMUA DALAM SATU CELL ===
import torch
...
# === Dataset ===
train_path = './data/train' # disesuaikan foldernya dari './dataset/train' ke './data/train'
data = []
...
# Tambahan: Mempertahankan Aspect Ratio (Mencegah Wajah Gepeng) sesuai EDA 1
...
train_transform = transforms.Compose([
    PadToSquare(), # Aplikasikan padding hitam terlebih dahulu
...
if len(df) > 0:
    ...
    print(classification_report(true_labels, preds, target_names=[idx2label[i] for i in range(6)]))
...
# Buang newline yang berlebihan dan buat format yang sesuai dengan Jupyter JSON source format
source_lines = [line + '\n' for line in new_cell_content.split('\n')]
# Menghapus trailing \n pada line terakhir untuk konsistensi
...
nb["cells"].append({
    ...
    "metadata": {},
    "source": [
        "## Super Train Restart (All in One Cell)\n",
        "Includes the **PadToSquare fix** to ensure aspect ratios aren't destroyed, as identified in EDA 1."
    ]
})
nb["cells"].append({
    "cell_type": "code",
